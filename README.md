...
지난 파트 2에서 배웠던 델리게이션 디자인 패턴이 무엇인지, 어떤 경우에 사용하는지, 실제 프로젝트에 어떻게 적용했는지 잘 학습했다면 테이블뷰에서 사용하는 데이터 소스와 델리게이트 역시 금방 이해할 수 있을 거에요. 테이블뷰에 원하는 정보를 표시하고, 사용자 선택에 적절히 반응하는 테이블뷰 구현을 위해 꼭 필요한 데이터 소스와 델리게이트에 대해 알아보도록 합시다.
